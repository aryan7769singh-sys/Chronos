...
export function ProjectNotesSection({ notes }: ProjectNotesSectionProps) {
    ...
    <Card className="border-border/60 bg-card/60 backdrop-blur-xs shadow-xs">
      <CardHeader className="p-4 pb-2 flex flex-row items-center justify-between space-y-0">
        <CardTitle className="text-sm font-semibold flex items-center gap-2">
          ...
          <span>Project Knowledge & Notes ({notes.length})</span>
        </CardTitle>
        <Link
          href="/notes"
      ...
      <CardContent className="p-4 pt-2">
        {displayNotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-4 text-center space-y-2 border border-dashed border-border/60 rounded-lg bg-card/30">
            <p className="text-xs text-muted-foreground">No notes linked to this project yet.</p>
            <Link
              href="/notes"
              ...
              return (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <h4 className="text-xs font-bold text-foreground line-clamp-1 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                      {n.title}
                    </h4>
